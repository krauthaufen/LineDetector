...
Target.create "Compile" (fun _ ->
    ...
    DotNet.build options "LineDetector.sln"
)
...
Target.create "Pack" (fun _ ->
    ...
    Paket.pack (fun o ->
        { o with
            ...
            ProjectUrl = "https://github.com/krauthaufen/LineDetector"
...
let releaseGithub() =
    ...
    | Some token -> 
        ...
        |> Fake.Api.GitHub.createRelease "krauthaufen" "LineDetector" notes.NugetVersion (fun p ->
            { p with
                Draft = true
...
Target.create "RunTest" (fun _ ->
    let options (integrated : bool) (o : DotNet.TestOptions) =
        let o = o.WithRedirectOutput false
        { o with
            Common = { o.Common with Environment = Map.add "INTEGRATED_GPU" (string integrated) o.Environment }
            NoBuild = true
            NoRestore = true
            RunSettingsArguments = Some "src/SimpeGL.Tests/test.runsettings"
    ...
    try
        using (Trace.traceTask "Integrated GPU" "integrated GPU") (fun _ ->
            DotNet.test (options true) "LineDetector.sln"
        )
    with e ->
        exns <- e :: exns

    try
        using (Trace.traceTask "Default GPU" "default GPU") (fun _ ->
            DotNet.test (options false) "LineDetector.sln"
        ) 
    with e ->
        exns <- e :: exns
    ...
    | [] -> ()
    | [e] -> raise e
    | es -> raise (AggregateException es)
